...
:::note[Deep Workflows Integration]

For advanced integration between Agents and Workflows, including bidirectional communication, automatic tracking, approval flows, and state synchronization, refer to the [Cloudflare Workflows Integration Guide](/agents/guides/cloudflare-workflows/). The guide covers the `AgentWorkflow` base class and workflow lifecycle management.
