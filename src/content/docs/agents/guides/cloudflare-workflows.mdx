...
title: Integrate Cloudflare Workflows
sidebar:
  order: 8
...
Integrate [Cloudflare Workflows](/workflows/) with Agents for durable, multi-step background processing while Agents handle real-time communication and state management.
...
- Familiarity with [Cloudflare Agents](/agents/)
- Familiarity with [Cloudflare Workflows](/workflows/)
...
Update your `wrangler.toml` or `wrangler.jsonc` to configure both your Agent and Workflows:
...
Methods added to the `Agent` class for workflow integration:
...
- [Human-in-the-Loop Agent Guide](/agents/guides/human-in-the-loop/)
